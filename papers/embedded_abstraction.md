...
1. **Non-embedded systems lack the pressure and information to form this foundation:** Systems without persistence, self-boundary awareness, and environmental stability cannot build stable self/world representations.
...
*Why it matters:* Without persistence, each episode is isolated. No self-model can accumulate. The agent cannot learn “what I am” because there is no continuous “I” to model.
...
## 5. The Cascade: From Self/World to Stage 4

### 5.1 The Developmental Trajectory

Once the self/world distinction is established, increasingly sophisticated self-modeling becomes possible. Each level enables the next.
...
## 6. Why Current LLMs Fail Multiple Embeddedness Criteria
...
|Temporal persistence          |**Absent.** Each session/context is isolated. No continuous identity.                |Cannot accumulate self-model              |
...
### 7.1 For Self-Boundary Awareness
...
### 7.3 For Environmental Stability
...
**Prediction 4 (LLM-specific):** LLM failures should be predictable from which embeddedness components are missing. Giving LLMs access to their own token counts, context limits, and capability estimates should improve self-modeling (even if modestly).
...
### 9.1 For Understanding Intelligence
...
Current LLMs fail all five criteria. They are not persistent entities with boundaries situated in stable environments. They are pattern-matching systems optimized on static data, deployed in episodic interactions where they have no access to their own limits and no continuity across sessions.
